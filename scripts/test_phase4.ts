...
async function runPhase4Tests() {
  ...
  const emailA = `studio.a.${timestamp}@apniestatetest.in`;
  const emailB = `studio.b.${timestamp}@apniestatetest.in`;
  ...
  const { data: signUpA, error: errA } = await clientA.auth.signUp({
    ...
    options: {
      data: {
        ...
        studio_name: 'Studio A Interiors',
  ...
  console.log('✅ User A created:', emailA, 'ID:', signUpA.user.id);

  // Bootstrap Org A
  const { data: orgAId, error: orgAErr } = await clientA.rpc('create_trial_organization', {
    org_name: 'Studio A Interiors',
    full_name: 'Aarav Studio A',
    phone: '+91 98200 11111',
    plan_name: 'studio'
  });
  if (orgAErr) console.log('Org A RPC note:', orgAErr.message);

  const { data: profileA } = await clientA.from('profiles').select('*').eq('id', signUpA.user.id).single();
  ...
  console.log('✅ User B created:', emailB, 'ID:', signUpB.user.id);

  // Bootstrap Org B
  const { data: orgBId, error: orgBErr } = await clientB.rpc('create_trial_organization', {
    org_name: 'Studio B Designs',
    full_name: 'Bhavna Studio B',
    phone: '+91 98200 22222',
    plan_name: 'studio'
  });
  if (orgBErr) console.log('Org B RPC note:', orgBErr.message);

  const { data: profileB } = await clientB.from('profiles').select('*').eq('id', signUpB.user.id).single();
